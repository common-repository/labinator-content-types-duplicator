(function($) {
    if (!window.Lct) window.Lct = {};

    Lct.CloneWidgets = {
        // Initialize
        init: function() {
            $('body').on('click', '.widget-control-actions .clone-me', Lct.CloneWidgets.Clone);
            Lct.CloneWidgets.Bind();
        },

        // Add Clone button to widgets control buttons
        Bind: function() {
            $('#widgets-right').off('DOMSubtreeModified', Lct.CloneWidgets.Bind);
            $('.widget-control-actions:not(.meks-cloneable)').each(function() {
                ...
                $clone.addClass('clone-me meks-clone-action')
                    .attr('title', lct_duplicate_widgets.title)
                    .attr('href', '#')
                    .html(lct_duplicate_widgets.text);
            ...
            $('#widgets-right').on('DOMSubtreeModified', Lct.CloneWidgets.Bind);
        },
