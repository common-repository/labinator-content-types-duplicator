<?php
add_action( 'admin_init', 'lct_duplicator_wpml_init' );

function lct_duplicator_wpml_init() {
	if ( defined( 'ICL_SITEPRESS_VERSION' ) ) {
		add_action('dp_duplicate_page', 'lct_duplicator_wpml_copy_translations', 10, 3);
		add_action('dp_lct_duplicator', 'lct_duplicator_wpml_copy_translations', 10, 3);
...
function lct_duplicator_wpml_copy_translations($post_id, $post, $status = '') {
	global $sitepress;
	global $duplicated_posts;
	
	remove_action('dp_duplicate_page', 'lct_duplicator_wpml_copy_translations', 10);
	remove_action('dp_lct_duplicator', 'lct_duplicator_wpml_copy_translations', 10);
		...
		foreach ($translations as $code => $details) {
			if ($code != $current_language) {
				if ( $details->element_id ) {
					...
					$new_post_id = lct_duplicator_create_duplicate( $translation, $status );
...
function duplicate_wpml_string_packages() {
	...
	foreach ( $duplicated_posts as $original_post_id => $lct_duplicator_id ) {
		
		$original_string_packages = apply_filters( 'wpml_st_get_post_string_packages', false, $original_post_id );
		$new_string_packages      = apply_filters( 'wpml_st_get_post_string_packages', false, $lct_duplicator_id );
